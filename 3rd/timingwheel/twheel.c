...
static struct TimeNode *timelist_clear(struct TimeList *l) {
...
static void timelist_add(struct TimeList *l, struct TimeNode *n) {
    (l->tail)->next = n;
    n->next         = 0;
    l->tail         = n;
}

static void timewheel_add_node(struct TimeWheel *TW, struct TimeNode *node) {
    ...
    if ((delay_time | TIME_NEAR_MASK) == (curr_time | TIME_NEAR_MASK)) {
        ...
        uint32_t i;
        for (i = 0; i < 4; i++) {
            if ((t1 | TIME_FAR_MASK) == (t2 | TIME_FAR_MASK)) { break; }
            t1 >>= TIME_FAR_SHIFT;
            t2 >>= TIME_FAR_SHIFT;
        }
...
static void timewheel_shift(struct TimeWheel *TW) {
    LOCK(TW);
            ...
            while (t) {
                if (t & TIME_FAR_MASK) {
                    struct TimeNode *next =
                        timelist_clear(&(TW->far[level][(t & TIME_FAR_MASK)]));
                    while (next) {
                        struct TimeNode *temp = next->next;
                        timewheel_add_node(TW, next);
                        next = temp;
...
timewheel_execute(struct TimeWheel *TW, time_Callback cb, void *ud) {
    LOCK(TW);
    struct TimeNode *next =
        timelist_clear(&(TW->near[(TW->curr_time & TIME_NEAR_MASK)]));
    UNLOCK(TW);
...
void timewheel_release(struct TimeWheel *TW) {
    LOCK(TW);
...
struct TimeWheel *timewheel_create(uint64_t t) {
    ...
    uint32_t i, j;
    for (i = 0; i < TIME_NEAR; i++) { timelist_clear(&TW->near[i]); }

    for (i = 0; i < 4; i++) {
        for (j = 0; j < TIME_FAR; j++) { timelist_clear(&TW->far[i][j]); }
    }
...
void timewheel_add_time(struct TimeWheel *TW, uint64_t handle, uint32_t t) {
    LOCK(TW);
...
void timewheel_update(struct TimeWheel *TW,
                      uint64_t          t,
                      time_Callback     cb,
                      void *            ud) {
